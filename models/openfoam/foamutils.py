...
def round2OrderMag(x,orderMag):
    xRounded = round(x/orderMag)*orderMag
    return xRounded


def floor2OrderMag(x,orderMag):
    xFloored = math.floor(x/orderMag) * orderMag
    return xFloored


def ceil2OrderMg(x,orderMag):
    xCeiled = math.ceil(x/orderMag) * orderMag
    return xCeiled
...
def calcBoundingBoxSizeOrderMag(bndBox):
    dX_orderMag = [0] * 3
    for i in range(3):
        dX = abs(bndBox[i+3] - bndBox[i])
        dX_orderMag[i] = math.pow(10.0, round(math.log10(dX/10.0)))
    return dX_orderMag
